...
fn round_trip(bson: &Document) -> IoResult<Vec<u8>> {
    ...
    assert_eq!(bson, &Document::from_bytes(&unwrapped[]).unwrap());
    bytes
}
...
fn test_null() {
    ...
    round_trip_bytes(&bson, vec![0x0b,0x00,0x00,0x00,0x0a,0x6e,0x75,0x6c,0x6c,
                                 0x00,0x00]);
}
...
fn test_i32() {
    ...
    round_trip_bytes(&bson, vec![0x0e,0x00,0x00,0x00,0x10,0x69,0x6e,0x74,0x00,
...
fn test_i64() {
    ...
    round_trip_bytes(&bson, vec![0x12,0x00,0x00,0x00,0x12,0x69,0x6e,0x74,0x00,
                                 0x00,0x5e,0xd0,0xb2,0x00,0x00,0x00,0x00,0x00]);
}
